...
# Replace using regex, escaping brackets for C# regex engine
$content = [regex]::Replace($content, "const GOOGLE_FONTS = \[[^\]]*?\];", $newFonts)
$content = [regex]::Replace($content, "const PAPER_PRESETS = \[[^\]]*?\];", $newPresets)

Set-Content -Path $filePath -Value $content -Encoding UTF8
Write-Host "Restoration success!"
