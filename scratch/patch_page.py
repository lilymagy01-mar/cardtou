...
for i, line in enumerate(lines):
    ...
    # 1. 속성 패널 교체 (약 1080라인 근처)
    if '{!selectedBlock ? (' in line:
        new_lines.append('                  {!selectedBlockId ? (\n')
        # 기존 블록 건너뛰기 (대략 1226라인 정도까지가 패널 내부임)
        # 닫는 중괄호를 찾을 때까지 스킵
        j = i + 1
        brace_count = 1
        ...
        # 새 하이브리드 패널 주입
        new_lines.append('''                    <div className="p-8 bg-gray-50 rounded-2xl text-center">
                      ...
                      <div className="space-y-1.5">
                        ...
                        <div className="grid grid-cols-2 gap-2">
                          <button 
                            ...
                            className={`py-2 text-[10px] font-bold rounded-xl border transition-all ${(selectedImageBlock as any).filter?.includes('grayscale') ? 'bg-orange-500 text-white border-orange-500' : 'bg-gray-50'}`}
                          >흑백</button>
                          <button 
                            onClick={() => updateImageBlockContent(selectedImageBlock.id, { filter: 'sepia(40%) saturate(120%)' } as any)}
                            className={`py-2 text-[10px] font-bold rounded-xl border transition-all ${(selectedImageBlock as any).filter?.includes('sepia') ? 'bg-orange-500 text-white border-orange-500' : 'bg-gray-50'}`}
                          >빈티지</button>
                          <button 
                            onClick={() => updateImageBlockContent(selectedImageBlock.id, { filter: 'brightness(1.2) contrast(1.1)' } as any)}
                            className={`py-2 text-[10px] font-bold rounded-xl border transition-all ${(selectedImageBlock as any).filter?.includes('brightness') ? 'bg-orange-500 text-white border-orange-500' : 'bg-gray-50'}`}
                          >화사하게</button>
                          <button 
